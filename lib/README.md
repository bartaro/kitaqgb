...
| `sprite_order.h` / `sprite_order.c` | Four selection-priority bands and cyclic OAM ordering, with signed screen clipping and up to 255 candidates. Emits at most 40 sprites and hides unused slots. | Include the header and compile the source; build shadow OAM in foreground and transfer during VBlank. See the [illustrated example and API reference](https://bartaro.github.io/kitaq-docs/en/gb-library.html#module-sprite_order). |
