# kitaqgb

KITAQGB is an open-source C toolchain for developing homebrew software targeting the Game Boy and Game Boy Color hardware.

KITAQGB is an independent project and is not affiliated with, endorsed by, sponsored by, or approved by Nintendo.

Game Boy and Game Boy Color are trademarks of Nintendo.

KITAQGBは、Game Boy / Game Boy Color向けの自作ソフトウェア開発を目的とした、独立系オープンソースCツールチェーンです。

KITAQGBは任天堂株式会社またはその関連会社とは無関係の非公式プロジェクトであり、任天堂による承認・後援・提携を受けたものではありません。

Game BoyおよびGame Boy Colorは任天堂の商標です。
