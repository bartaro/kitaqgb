...
// Stop the LCD after a VBlank wait, load the supplied font and clear the
// background map, then restore the palette and LCD configuration.
void m_init() {
    __wait_vblank(); M_LCDC = 0; M_BGP = 0xE4; M_OBP0 = 0xE4;
    __vram_copy(0x8000, manual_font, 2048);
    __vram_fill(0x9800, 0, 1024); M_LCDC = 0x91;
}
// Write one character code directly as a background font tile.
// This support font uses character codes as tile IDs; do not replace its tile order independently.
void m_put(u8 x,u8 y,u8 ch) { __settile_xy(x,y,ch); }
// Wait for the next VBlank without changing the system library frame counter.
